...
@PrepareForTest({ RequestBuilder.class, JsonUtils.class, ContentType.class, AbstractZRequestRunner.class })
public abstract class AbstractZssRequestRunnerTest extends ZoweApiTest {

    // TODO NOW - refacotr with zosmf version

    static final String BASE_URL = "https://dummy.com/zss/";

    @Mock
    protected ZssConnector zssConnector;
    ...
    public void setUp() throws Exception {
        when(zssConnector.getFullUrl(anyString())).thenAnswer(new org.mockito.stubbing.Answer<URI>() {
            @Override
        ...
        when(zssConnector.getFullUrl(anyString(), anyString())).thenAnswer(new org.mockito.stubbing.Answer<URI>() {
            @Override
            public URI answer(org.mockito.invocation.InvocationOnMock invocation) throws Throwable {
                Object[] args = invocation.getArguments();
    ...
    protected void verifyInteractions(RequestBuilder requestBuilder, boolean path)
        ...
        verify(zssConnector, times(1)).request(requestBuilder);
        if (path) {
            verify(zssConnector, times(1)).getFullUrl(anyString(), anyString());
        } else {
            verify(zssConnector, times(1)).getFullUrl(anyString());
        }
        verifyNoMoreInteractions(zssConnector);
    }
    ...
    protected String loadTestFile(String relativePath) throws IOException {
        return loadFile("src/test/resources/zssResponses/" + relativePath);
    }
