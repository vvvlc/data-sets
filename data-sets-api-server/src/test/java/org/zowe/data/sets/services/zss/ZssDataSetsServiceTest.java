...
@PrepareForTest({ ZssDataSetService.class })
public class ZssDataSetsServiceTest extends ZoweApiTest {

    @Mock
    ZssConnector zssConnector;

    ZssDataSetService dataService;

    @Before
    public void setUp() throws Exception {
        dataService = new ZssDataSetService();
        dataService.zssConnector = zssConnector;
    }
...
//        when(runner.run(zssConnector)).thenReturn(expected);
...
//
//        ListDataSetMembersZosmfRequestRunner runner = mock(ListDataSetMembersZosmfRequestRunner.class);
//        when(runner.run(zssConnector)).thenThrow(expectedException);
//        PowerMockito.whenNew(ListDataSetMembersZosmfRequestRunner.class).withArguments(dataSetName).thenReturn(runner);
//
...
//    }
    ...
    public void testListDataSetAttributesRunnerValueCorrectlyReturned() throws Exception {
        ...
        ListDataSetsAttributesZssRequestRunner runner = mock(ListDataSetsAttributesZssRequestRunner.class);
        when(runner.run(zssConnector)).thenReturn(expected);
        PowerMockito.whenNew(ListDataSetsAttributesZssRequestRunner.class).withArguments(filter).thenReturn(runner);
...
//        when(runner.run(zssConnector)).thenReturn(etag);
...
//        when(runner.run(zssConnector)).thenReturn(name);
...
//
//        verify(runner).run(zssConnector);
//    }
//
//    @Test
